...
Project Goal: To rapidly prototype the core loop of a real-time management/defense game where the player manages a DevOps shop, hiring engineers (Debs) to maintain server infrastructure and battle "Tech Debt."
...
The player manages a small server farm on an isometric grid. They must allocate worker NPCs ("Debs") to repair active server failures ("Fires") and maintain existing infrastructure (Tech Debt) while ensuring overall uptime to generate income.
...
Player views the map, purchases new servers, hires new Infrastructure Debs (S5), allocates Debs to permanent tasks, and hits "Start Day."

Buy/Sell Servers, Hire Debs, Assign Debs, Start Day button.

B. Day Phase (Real-Time)

Servers generate money. Random failure events ("Fires") spawn. The player manually redirects Debs to put out fires or complete tasks. Debs expend Engineering Hours (EH) to complete tasks.

Real-time observation, Click-to-Move orders.

C. Night Phase (Paused)

Day concludes. All resource gains (Money, Research) are tallied, and Engineering Hours used are summarized. Infrastructure Debs are automatically paid (Salary). Player engages with the Research Panel (placeholder).
...
S1: NPC Management (The Debs - Infrastructure Team)

Entity: A single "Deb" NPC prefab using the 8-directional pixel art character asset.

Team Size Cap: The maximum number of controllable Infrastructure Debs is tied to the number of days survived (e.g., Max Debs = 1 + floor(Days Survived / 7)).

Movement: Simple A* pathfinding (using Unity NavMesh or a simple grid script) that allows the Deb to pathfind from their current location to a clicked destination (a Server or a Task).
...
Task Switching Penalty: If the player interrupts a Deb currently working on a Low Priority task to assign a High Priority task (manual override), the Deb incurs a fixed time delay (e.g., 1 second) before moving, simulating context switching/re-prioritization overhead. The penalty must be visible via UI text over the Deb.

Interactions: When a Deb reaches a target Server:
...
Toggle Mechanism: Player clicks server; Deb runs to it. On arrival, Deb flips the state (ON to OFF or vice-versa).

Tech Debt: Each ON server has a visible Tech Debt counter that increases over time. This counter is converted into an increasing chance of a fire event when it hits certain thresholds. A dedicated Deb assigned to Maintenance expends EH (S2) to slowly reduce this counter.
...
Action: The player can hire a candidate if they have enough money and are below the Team Size Cap (S1). Once hired, the Deb is added to the controllable team.

4. Future Scope (V2.0+)

These features are out of scope for the MVP prototype but are vital for the full game.

Skill-Based Efficiency (EH): Debs' task speed (EH effectiveness) will be based on their skill (Junior/Senior).

Tech Tree Implementation: Implement the research mechanic (e.g., Debs assigned to "Research Station" generate Research Points). Unlockable nodes include:
...
Advanced Hazards: Introducing different types of events beyond just fire (e.g., DDoS attacks requiring a security-focused Deb, Database corruption requiring a special repair tool).
...
A player can view the max Infrastructure Team Size based on the current game day (S1).

In the Planning Phase, a player can view candidates' Salaries and Skills and hire a Deb up to the team cap (S5).
...
A Task Switching Penalty is displayed and enforced when interrupting a Deb's work.
